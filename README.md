...
- РЕШЕНИЕ: Использовать одинаковый порядок Лока во всех методах и операциях с мьютексами. Использовать сложные мьютексы которые можно лочить более одного раза. Использовать Спин_локи 
3) *Livelock (Активная блокировка)* [Livelock считается подслучаем Starvation]
...
4) *Starvation (Голодание потоков)*
...
5) *Race condition (Состояние гонки)*
- Когда время или порядок событий влияют на корректность результата
- РЕШЕНИЕ: Блокировки потоков, которые обгоняют другие потоки, в не нужный для нас момент, построение очереди из потокв для верной последовательности операция
...
- РЕШЕНИЕ 1: *АТОМАРНЫЕ операции* - Если операция НЕ Атомарна — то она состоит из нескольких шагов. Во время АТОМАРНОЙ операции НИКТО не может "втиснуться" перед ней или выполниться одновременно с нею.
- РЕШЕНИЕ 2: Блокировки через Мьютексы=std::mutex=NSLock(lightweight-lock with .lock()/.unlock())/Крит.секции/Локи/Кондвары/Semaphore/Serial queue и т.д. потоков, которые обгоняют другие потоки, в не нужный для нас момент, построение очереди из потокв для верной последовательности операция
