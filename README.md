...
- Ответ: std::deque
- У QList быстрая вставка по индексу за О(1)
- У QList долгая вставка в середину за О(N)

- В Документации по Кутэ есть статья с описанием класса QList
- В описании есть Параграф с названием "Detailed Description"
- В нём есть Абзац, про то, что QVector должен быть вашим контейнером по умолчанию для решения большинства задач. QVector<T> чаще всего даст вам лучшую производительность чем QList<T>, ибо QVector<T> всегда хранит элементы последовательно в памяти, в то время как QList<T> будет аллоцировать свои элементы в куче если только не будет выполнено 1. sizeof(T) <= sizeof(void*) и 2. T было определено как либо Q_MOVABLE_TYPE или Q_PRIMITIVE_TYPE при помощи Q_DECLARE_TYPEINFO.
- В конце этого абзаца есть **ССЫЛКА See the Pros and Cons of Using QList for an explanation**. По сылке статья https://marcmutz.wordpress.com/effective-qt/containers/#containers-qlist

QList воплощение фарисейства и лицемерия среди всех Qt контейнеров. Лучший пример того как благими намерениями и поступками в виде реализации контейнера QList была выстроена дорога куда-то явно не туда, но всё же выстроена.

QList это **НЕ std::list**. Реализован он как **так называемый array list**, хотя по всем своим свойствам, что будут описаны ниже он будет довольно близок к STL-контейнеру **std::deque**, но всё же и им сторого говоря не является. В общем QList фактически непрерывный кусок void*-ов, с кусочками свободных мест дло и после, для предостваления возможности как добавления в начало (очень редко), так и добавления в конец (очень часто). The void* slots contain pointers to the individual elements (which are copy-constructed into dynamic memory, ie. with new), unless the element type is movable, i.e. declared to be Q_MOVABLE_TYPE, and small enough, i.e. not larger than a void*, in which case the element is placed directly into the void* slot.

ПЛЮСЫ:
- С положительной стороны, QList действительно экономит память, когда мы говорим о количестве сгенерированного кода. Это потому, что QList — это всего лишь тонкая обертка вокруг внутреннего класса, который работает с памятью  содержащихся в нём void*-элементов. Это приводит к более компактному коду, потому что весь код управления памятью разделяется между QList-ами разных типов.
- QList также допускает достаточно быстрые вставки в середину и достаточно эффективное увеличение контейнера (нужно перемещать только указатели, а не сами данные, поэтому QList всегда может использовать realloc() для своего увеличения). Тем не менее, эта эффективность не будет намного выше, чем можно ожидать от QVector<void*>, а векторы, как правило, не славятся быстрыми вставками в середину.
МИНУСЫ:
- С другой стороны, QList — настоящий пожиратель памяти, когда дело касается хранения большинства типов данных.
- У контейнера имеются 2 нюанса:
- ПЕРВЫЙ, если тип элемента movable и достаточно мал (см. выше), QList расходует память, когда размер типа элемента меньше sizeof(void*): sizeof(void*)-sizeof(T), если быть точным. Это потому, что каждый элемент требует как минимум sizeof(void*) памяти.
- Другими словами: QList<char> использует в 4× раза болше памяти на 32-битной платформе, чем QVector char-ов (QVector<char>) и соотвественно использует в 8× раза болше памяти на 64-битной платформе, чем QVector char-ов (QVector<char>)!
- ВТОРОЙ, если тип элемента либо not movable, либо слишком большой (sizeof(T) > sizeof(void*)), он выделяется в куче. Таким образом, за каждый элемент вы платите накладные расходы за выделение в кучи (зависит от allocator, обычно от нуля до дюжины или двух байт), плюс 4/8 байт (для х32/х64 битных ОС соотвественно), которые требуются для хранения указателя.
- Только если тип элемента movable и имеет размер sizeof(void*), QList является хорошим контейнером. Так происходит, по крайней мере, с самыми важными из неявно общих типов Qt (QString, QByteArray, а также QPen, QBrush и т. д.), за некоторыми заметными исключениями. Вот список типов, которые в Qt 4.6.3 задокументированы как неявно общие, и каждый по своему до какой-то степени является хорошим для использования в QList:
...
- META_MACROS
- Q_MOC_RUN
- Если его забыть написать Q_OBJECT, получим МНОГО ошибок компиляции
...
### Как создать поток
**РЕШЕНИЯ**:
- Использовать QtConcurrent::run
- Использовать QFurute
- Поместить логику в специальный Worker класс, отнаследованный от QObject, поместить Worker в стандартный QThread через moveToThread() и управлять Worker-ом с помощью сигналов. 
- Сделать наследника от QThread и переопределить run()

### ПОТОК APPLICATION THREAD  PARENT CHILD GUI THREAD RENDERING THREAD ПОТОК ПРИЛОЖЕНИЯ
...
|______________________________________________|
  ...
  ^                               Events
  |                                  |
  | Events __________________________|____________
  |        | QThread                 |            |
  |        | __________            __V__________  |
  |--------|-| QObject |<-\       |QEvent Queue | |
  |        | -----------   \Calls --------------- |
  |        |                \        | Reads      |
  |        | __________      \_______V_____       | 
  |--------|-| QObject | <----| QEventLoop |      |
  |        | -----------     /--------------      |
  |        | __________     /                     |
  |--------|-| QObject | <-/                      |
...
- а вот QObject всегда имеет ВСЕГДА ОДИН родительский поток и он всегда ровно один. 
- **«Главный» (main thread)**, который в случае с UI-приложениями так же становится **GUI thread**.
- QApplication (QCoreApplication / QGuiApplication) который обслуживает основной event loop ориентированный на работу с сообщениями от операционной системы
- «Главным» станет тот поток, который собственно создал объект QApplication.
- До того как выполнится QApplication a.exec(); фактически ещё не будет работать Even Loop основного приложения
- Каждый объект в Qt «принадлежит» какому-то потоку. И какому, именно потоку, тут В Qt приняты следующие соглашения:
- 1. Все «дети» любого «родителя» всегда живут в том же потоке что и родительский объект (Правило «созданные объекты наследуют текущий поток»)
-- ЕСЛИ сделать setParent к объекту живущему в другом потоке в Qt просто молча фейлится (в консоль пишется предупреждение)
-- СУТЬ: удаление «детей» при гибели живущего в другом потоке родителя является очень нетривиальной и склонной к трудно отлавливаемым багам проблемой
- 2. Объект у которого при создании не указан родитель живет в потоке который его создал
-- Из-за этого QThread (как объект) живет в другом потоке чем собственно тот поток который он контролирует (и в силу правила 1 не может владеть ни одним из объектов созданных в этом потоке).
...
- 3. В главном GUI-thread потоке должны «жить» все объекты связанные с GUI, то есть Виджеты. 
-- Объекты виджетов **Widgets нельзя moveToThread()** переместить в другой поток => **получим terminate**. Они являются детьми основного приложения, а оно есть родитель, и живёт в основном GUI-thread-е (см. 1 пункт)
-- Widget-type object может жить только в главном потоке, попытка «переместить» такой объект в любой другой автоматически зафейлится. В силу этого даже существует специальный метод QObject::isWidgetType()
- 4. Отрисовка объектов в Qt происходит в специально выделенном для этого потоке, RENDERING THREAD
-- Объекты в qt неявно делятся на «фронт-енд» (обработки событий выполняет именно фронт-енд) который видит программист и обычно скрытый от него «бэк-энд» (Бэк-энд живет в rendering thread) который собственно осуществляет реальную отрисовку. 
- 5. Помним, что Q_OBJECT не работает с template-классами. Данное ограничение обходится написанием дополнительнго класса-обёртки

// TO DO

### Минус наследования от QThread-а и переопределения run() метода

https://habr.com/ru/post/467261/

Одним из неочевидных подводных камней здесь является то что в Qt у потока, вообще говоря, может вообще не быть Dispatcher-а и соответственно ни одного EventLoop-а. Объекты принадлежащие этому потоку **НЕ** будут реагировать на посылаемые им события. Поскольку **QThread::run() по умолчанию вызывает QThread::exec() внутри которого** как раз реализован стандартный **EventLoop**, то с этой проблемой часто сталкиваются те кто пытается определить свою собственную версию run() отнаследовавшись от QThread-а. Подобный вариант использования QThread в принципе вполне валиден и даже рекомендуется в документации, но идет вразрез с общей идеей организации кода в Qt описанной выше и частенько работает не так как ожидают того пользователи.
- Характерной **ошибкой** при этом является **попытка останавливать подобный кастомный QThread (наследник QThread-а, переопределяем run()) путем вызова QThread::exit() или quit()**. Обе эти функции направляют сообщение в **QEventLoop**, но если **в потоке этого QEventLoop просто нет**, то и обрабатывать их, естественно, некому. В результате неопытные пользователи пытаясь «починить неработающий класс» начинают пытаться использовать «работающий» QThread::terminate, чего делать категорически нельзя. Имейте в виду — в случае 
- **ЕСЛИ Вы переопределяете run() и не используете стандартный event loop, то** механизм **выхода из потока** придется предусмотреть самостоятельно — к примеру воспользовавшись специально добавленной для этого функцией **QThread::requestInterruption()**.
- Правильнее, впрочем, просто **НЕ Наследоваться от QThread** если Вы не собираетесь действительно реализовывать какую-то специальную новую разновидность потоков и **либо использовать** специально созданный для подобных сценариев **QtConcurrent**, **либо поместить** логику в специальный **Worker Object отнаследованный от QObject, поместить последний в стандартный QThread и управлять Worker-ом с помощью сигналов**. 
...
### Как принудительно запустить прокрутку EventLoop-а
- app->processEvents


### For correct rowCount() => use fetchMore() on QSqlTableModel with a SQLite DB 

https://stackoverflow.com/questions/26369420/when-or-how-to-use-fetchmore-on-qsqltablemodel-with-a-sqlite-database-for-rowc
...
- Сигналы можно коннектить с сигналами - connect(sender, signal, reciever, signal)
- Выслать сигнал можно И БЕЗ помощи ключевого слова emit, а просто написав вызов метода.
...
- SLOT по умолчанию НИКАКОЙ (требует самим дописать модификатор доступа) Они определяются в классе как <b>virtual</b>, <b>private slots:</b>, <b>protected slots:</b> или <b>public slots</b>:
...
### ConnectionType type, 4 типа соединения 4 возможных значения:
- **В ОБЩЕМ СЛУЧАЕ:**
...
# АНДРОЙД ANDROID Qt Андроид Qt Qml Андроид Qml Qt Android Qt Qml Android Qml 

- Мы деплоили наш клиент под андройд. Нужно было запустить клиент на планшете и частично хотя бы вкладку настроек запустить на Смартфоне.

ПЕРЕД СБОРКОЙ
- Для этого по мимо QtCreator-а через MaintanenceTool надо было отдельно поставить Android Studio ибо в QT не было всего что нужно для КОМПИЛЯЦИИ проекта.
- Потребовалось настроить: Andrid NDK + SDK все минимально необходимые + все прекомпиленные QT-штые штуки для будущей сборки.
- На андройдовском устроостве **Включить РЕЖИМ РАЗРАБОТЧИКА** (много раз на версию жмём)
- Настроить соединение с устройством по проводу (можно было и через WiFi но не пробовал)
- Деплой приложения по проводу (порт откроется) прямо на устройство происходил на прямую и соотвественно с ПК в андройд Студии приложение можно через отладчик даже запускать и отлаживаться

CБОРКА на Виндоуз и на Линукс
- Конечный проект компилился в файл с расширением *.APK - туда собирается всё. Она получалась БОЛЬШАЯ, она кушала МНОГО памяти.
- Кросскомпиляция под АРМ_v7 / АРМ_v8 + смотреть на версию и архитектуру установленных кутешных библиотек дополнительно
- Вопрос был ещё в том, что мы пробовали собрать браузерную версию с использованием WebAssembly, но я почти ничего из этого уже сейчас не вспомню.
- При сборке приходилось обращать внимание НА:
- внимание на Плагины обращать (для 5/6 версии было не просто понять, что в APK попали ВСЕ нужные тебе плагины)
- Нужно было смотреть кучу флагов, чтобы всё правильно компилилось, собиралось и работало по итогу
- В 6 Кутэ много новых макросов добавили, например для того, чтобы подключить QML
- qt_add_qml_module, #include <qqml.h>, class BackEnd : public QObject { ... QML_ELEMENT }
- Ранее нужна была связка qmlRegisterType<MyClass>("my_company", 1, 0, "MyClass");, qmlRegisterUncreatableType, qRegisterMetaType<const MyClass*>("const MyClass*"); qmlRegisterUncreatableType<const MyClass>("Test", 1, 0, "MyClass", "Don't create it");
- QML_ELEMENT , QML_NAMED_ELEMENT , QML_SINGLETON , и др. макросы https://doc.qt.io/qt-6/qtqml-cppintegration-definetypes.html

Widgets и QtQuick.Controls
- С Виджетами было плохо:
- проблема с шрифтами были 
- проблемы с вертикальным экраном (переключение с вертикального на горизонтальный форматы)
- проблемы с масштабированием были 
- проблемы с разрешениями были => рендеринг
- проблемы с отображением чегото особенного чего хотелось кастомного были, приходилось плясать с бубном, чтобы понимать что возможно, а что всё же нет.
- С QML с 6 версии Кутэ, сами контролы стандартные стали более удобные, более функциональные, более адаптивные + matherial theme стали поддерживать более менее полноценно и ещё какие-то плюсы были
- Сеть работала на Андройде. Сервак затаскивать не надо было. Затаскивать БД тоже.
...
O: Open-Closed Principle (Принцип открытости-закрытости). Программные сущности (классы, модули, функции) должны быть **ОТКРЫТЫ для РАСШИРЕНИЯ**, но **ЗАКРЫТЫ для МОДИФИКАЦИИ**. (вместо switch-case makeSound, добавить виртуальный метод makeSound / интерфейс для класса)
...
## Имеем проблемный код, класс с кучей одинаковых по своей сути методов
// S != GOD_OBJECT - нарушает принцип единственной ответственности
```
// GODObject - хоть и не Синглтон, но может 
class IOManager {
public:
 ...
 void SaveToServer   (const std::string& url); // over FTP
 void ReadFromServer (const std::string& url); // over UDP
};
```
## // Улучшение 1 // Улучшим S = Single Responsibility
```
class FileManager {
public:
 void SaveToFile  (const std::string& filePath);
...
- Закрыт для ИЗМНЕНЕИЯ / Открыт для РАСШИРЕНИЯ
- Если мы захотим добавить какой-то новый функционал,
- Например, будет Объект для которого будет акутально только 
- READ, но не будет актуален Сейв некий SettingsManager()
- SAVE допустим будет бросать Исключение, о том, что он Незаимплементирован

L - Принцип подстановки Барбары Лисков
- Код который использует Базовый класс, так же должен использовать класс Наследник без особых проблем и не задумываться о том какой класс он использует

## // ПЛЮСЫ, когда мы разбили проект на много мелких файлов и классов
- Код стало  проще ЧИТАТЬ + ПОДДЕРЖИВАТЬ
- Код проще если надо ИЗМЕНЯТЬ (меньше affected частей)
- Код станет проще ТЕСТИРОВАТЬ
- Работа по Принципам, которые ПОНЯТНЫ и другим программистам
- Люди проще и быстрее разберутся, увидят известный подход,
- быстрее и проще будет происходить ВХОД в проект и Резолвинг КОНФЛИКТОВ при МЁРДЖАХ
- Работа над множеством мелдких файлов даёт больше свободы разрабам в процессе командной разработки
- Проще раздавать задачи и разделать зоны отвественности за код или участки кода проекта
- 3 разраба работающие в 3 разных файлах смёрджат свой результат легко
- 3 разраба работающие а 1 файле в рамках ГОД Обжект Класса будут мёрджить свои результаты долго и болезненно
   

### POSA Pattern Oriented Software Architecture

Пятитомник по архитектуре POSA или Pattern Oriented Software Architecture описывает варианты общей структуры приложения, и затем уходит в детали.

### GRASP

GRASP это 9 шаблонов проектирования =  5 основных + 4 дополнительных шаблонов.
...
- Pure Fabrication
- Indirection - Перенаправление - Слабое зацепление между элементами системы (и возможность повторного использования) через посредника. ПРИМЕР: MVC
- Polymorphism -  позволяет обрабатывать альтернативные варианты поведения на основе типа. При этом, альтернативные реализации приводятся к обобщенному интерфейсу.
- Protected Variations - Устойчивость к изменениям - Шаблон защищает элементы от изменения другими элементами (объектами или подсистемами) с помощью вынесения взаимодействия в фиксированный интерфейс, через который (и только через который) возможно взаимодействие между элементами.
...
### Переопределение преинкремент и постинкремент / преинкремента и постинкремента
	
- Чтобы различить их, соблюдается следующее правило: префиксная форма оператора объявляется точно так же, как и любой другой унарный оператор; Постфиксная форма принимает дополнительный аргумент типа int .
	
```
type operator++() // операторная функция, которая перегружает префиксный оператор ++ (++X)
{ } // тело операторной функции
type operator++(int d) // операторная функция, которая перегружает постфиксный оператор ++ (X++)
{ } // тело операторной функции
...
- Концепция RAII (Запрос ресурса при инициализации) = ресурс запрашивается в конструкторе и освобождается в декструкторе. Суть идиомы RAII в том, что класс инкапсулирует владение (захват и освобождение) некоторого ресурса — например, открытого файлового дескриптора. Когда объекты-экземпляры такого класса являются автоматическими переменными, гарантируется, что когда они выйдут из области видимости, будет вызван их деструктор — а значит, ресурс будет освобождён.
...
### Размер объекта object size of object
...
5) (0) - методы внутри объекта не влияют на размер объекта (static) - static-поля НЕ ЯВЛЯЮТСЯ ЧАСТЬЮ КЛАССА
...
### override
- Классов много, объектов много.
- Бейз - Чайлд1 - ПодЧайлд
- в Бейз есть виртуальный метод явно обозначенный словом virtual в остальных Чайлде и Подчайлде просто объявления методов. Т.к. в подклассах есть переопределение методов, и у метода в базовом классе стоит virtual, и методы в классах наследниках и поднаследниках имеют одинаковую сигнатуру, то
- 1. компилятор неявно припишет КЛ.слово virtual всем методам во всех классах наследниках, которые будут иметь (методы) ту же сигнатуру, что и виртвульная функция из базового класса
- 2. при работе с данным методом через указатель на базовый класс Base * b = new Derived(); b->fun(); fun вызовется из Наследника, ибо указатель хоть и типа Бейз*, но в том учатке памяти на который этот указатель указывает Ньюкнулся именно объект Дерайвд класса и именно из него будет вызван метод согласно принципу Полиморфизма.
### Для чего хорош override:
- Упрощает тестирование кода и чтение другими программистами / тестировщиками. Он впринципе не нужен, но это "синтаксический сахар" добавляющий лишь смысловой нагрузки, говорящий нам о том, что этот метод переопределён, и что есть где-то базовый класс (класс выше по уровню нгасоледования) в котором этот метод с такой же стигнатурой объявлен как виртуальный. БЕЗ этого слова,  т.е. допустим мы не увидели, что у класса есть родитель, и если программист не написал НИ override у этого метода в наследнике НИ virtual, то мы могли бы упустить из виду эту информауцию, что оказывается этот метод переоепределённый и виртуальный.
- override во время компиляции РУГНЁТСЯ, если не найдёт родительский класс в котором должен будет быть метод с ключевым словом virtual и с сигнатурой метода, перед которым стоит слово override в классе наследнике. Т.е. он выполняет защиту от ошибки, если кто-то случайно поменял виртуальный метод в наследнике так, что виртуальность данного метода "сломается" для класса наследника в котором этот метод поменяли, override проследит за этим и ругнётся, если это случится.
...
# std::string 
- Строка тоже контейнер из символов, и тоже динамический массив, по своей работе больше всего напоминает ВЕКТОР.
- Метод .data() - вернёт const char * - Получение указателя на кусок памяти где хранится строка. Указатель этот валиден до тех пор пока мы не РЕАЛЛОЦИРУЕМ / перекопируем весь вектор в случае его расширения (Когда измется именно CAPACITY)
- В силу SSO / SBO (Smal String/Buffer Optimization) у строки в момент её создания CAPACITY = 15 и пока строка не более 15 символов она хранится на стеке в виде массива, КАК ТОЛЬКО выходит за CAPACITY > 15, то начинает хранится в Куче
- Вставка .append() в КОНЕЦ строки занимет - **АМОРТИЗИРОВАННОЕ** O(1), ибо пока строку не нужно расширять (size <= capacity), то новый символ/подстрока за О(1) добавляется в конец строки без реаллокации, как только надо вставить символ или подстроку в конец, когда capacity уже не хватает (new size > current capacity), строка РЕАЛЛОЦИРУЕТСЯ (string выделяет себе памяти в ДВА раза больше старого capacity в новом участке памяти, и копирует все значения из старого места (после копирования удаляет старую память) в новый реаллоцированный участок памяти, что может привести к оценке скорости в О(n)
...
- Метод .data() - вернёт const char * - Получение указателя на этот самый смежный кусок памяти. Указатель этот валиден до тех пор пока мы не РЕАЛЛОЦИРУЕМ / перекопируем весь вектор в случае его расширения
...
# Типы итераторов Итераторы Iterators:
...
### Абстрактный класс и Интрефейс различие. Отличие Абстрактного класса от Интерфейса
Короткое различие. 
- **ГЛАВНОЕ** у **Интерфейса** — **ВСЕ** его нереализованные **методы ПУБЛИЧНЫЕ**, а у **Абстрактного класса** — могут быть **методы и НЕ ПУБЛИЧНЫЕ**, то есть и private и protected и уже только после этого:
- **ВТОРОЕ** у **Интерфейса** — **ВСЕ** нет переменных в классе только методы, а у **Абстрактного класса** — хоть это и противоречит основномы смыслу абстрактного класса, но он может иметь переменные внутри себя.
...
- Интерфейс — это абстрактный класс, у которого (1) **НЕ РЕАЛИЗОВАН ни один метод** и (2) **ВСЕ МЕТОДЫ ПУБЛИЧНЫЕ** и нет переменных класса.
...
- Базовая гарантия безопасности исключений - если возникло исключение, то все объекты остаются в согласованном состоянии, УТЕЧЕК ресурсов НЕ ПРОИЗОЙДЁТ
- Строгая гарантия безопасности исключений - если во время операции возникло исключение, то программа останется в том же состоянии, которое было до начала выполнения операции.
- ДРУГИМИ СЛОВАМИ - строгая гарантия исключений обеспечивает **ТРАНЗАКЦИОННОСТЬ операций**, когда мы получаем либо все, либо ничего. Будет полный откат всех действий до момента вызова кода, изза которого вызвалось исключение, утечки памяти не случатся.
- ДРУГИМИ СЛОВАМИ - если при выполнении операции возникает исключение, то это не должно оказать какого-либо влияния на состояние приложения.
...
### ПОЧЕМУ Деструкторы помечены неявно noexcept(true)
Any user-defined destructor is noexcept(true) by default

- **ОТВЕТ И ПРИЧИНА**: terminate() вызывается, т.к. ОБРАБОТЧИК exeption-ов НЕРЕЕНТЕРАБЕЛЕН НЕРЕИНТЕРАБЕЛЕН.
- **ОТВЕТ И ПРИЧИНА**: Плюс если обработчик исключений будет РЕИНТЕРАБЕЛЕН РЕЕНТЕРАБЕЛЕН, то процесс обарботки исключений превартится в ЧЕРЕЗЧУР сложный процесс, до которого комитет не хотел доводить.
- **Реентерабельная процедура** или повторно входимая функция – это функция, которая может быть вызвана из другого потока до завершения её исполнения в предшествующем вызове в первом потоке. При этом такая такая функция должна исполняться корректно, иначе она не будет обладать свойством реентерабельности. 

https://www.ixbt.com/soft/intel-parallel-composer.shtml

- **ПОЧЕМУ 1** - : Деструкторы объкектов вызываются во время 
размотки стека в тот момент когда бросилось исключение. 
Если деструктор бросает исключение, 
и его вызов был вызван тем что бросилось исключение, 
то вызовется std::terminate(), которая по умолчанию
вызовет std::abort(). Когда std::abort() вызовется, 
удаление объектов прекратится, что приводит к 
неконтролируемому состоянию программы и к UB = undefined behavior. 

- WHY: Object destructors are likely to be called during stack unwinding
as a result of an exception being thrown. 
If the destructor itself throws an exception, 
having been called as the result of an exception being thrown, 
then the function std::terminate() is called with the default effect of 
calling std::abort(). When std::abort() is called, 
no further objects are destroyed, resulting in an 
indeterminate program state and undefined behavior. 
Do not terminate a destructor by throwing an exception. 

- **ПОЧЕМУ 2** - Из-за виртуальных функций, и виртуальных деструкторов
Удаление объектов полиморфного класса, если бы в деструкторах
бросались бы исключения, было бы слишком просто нарваться на
проблемы с выбросом двойного исключения, которое приводит к terminate()

- **ПОЧЕМУ 3** - По аналогии с тем как бросание исключений в конструкторе
приводит к тому, что конструктор не отработает полностью и объект
полностью не сконструируется, так же и с деструктором. Обрыв вызова деструктора
приведёт к незавешённому удалению объекта.
...
1) Не использовать join() / detach() - НЕ НАПИШЕМ ИХ И ПРОГРАММА УПАДЁТ ВСЕГДА. Не важно успеет выполнится функция в потоке t1(func()) ДО того как закончит работу основной поток или нет, ПАДЕНИЕ БУДЕТ ВСЕГДА.
...
### Как получить исключение / поймать исключение, вылетевшее в другом потоке / исключение в другом потоке ?
...
### С++ 17
- Автоматический вывод **параметров шаблонов** ((1) функции вида std::make_pair больше не нужны: смело пишите выражения std::pair{10, "hello"s}, компилятор сам выведет тип | (2) шаблонные RAII вида std::lock_guard<std::mutex> guard(mutex); станут короче: std::lock_guard guard(mutex);)
- Класс **string_view** для параметров-строк ( (1) Класс string_view хорош тем, что он легко конструируется и из std::string и из const char* без дополнительного выделения памяти. А ещё он имеет поддержку constexpr и повторяет интерфейс std::string. Но есть минус: для string_view не гарантируется наличие нулевого символа на конце. (2) будьте осторожны с возвратом string_view из функции: это может привести к проблеме висячих ссылок (англ. dangling pointers))
- Классы **optional и variant** ((1) используйте тип variant вместо any везде, где это возможно (2) предпочитайте optional<T> вместо unique_ptr<T> для композиции объекта T, время жизни которого короче времени жизни владельца)
- **std::filesystem**
	
### c++20

- **Корутины** - Корутины описывались как взаимодействующие программы (но не как конкурентные программы), выполняющиеся в одном потоке.
```
X coroutine()
{
    co_yield "Hello ";
...
- **Ranges** - Диапазоны значений / переменных - constant time copy/move/assignment | у них есть все методы, что есть у контейнеров стандартных .beign(), .end(), rbeign(), size(), empty(), data() и ranges::view, ranges::input_range, ranges::output_range, ranges::forward_range т.д.  (ranges::random_access_range - specifies a range whose iterator type satisfies)
- **Расширение возможностей шаблонов** (Концепт — это имя для ограничения. Мы свели его к другому понятию, определение которого уже содержательно, но может показаться странным: Ограничение — это шаблонное булево выражение.)
...
- Многопоточка позволяет использовать всю мощь процессоров
- (!!!) Плохо написанная многопоточная программа МОЖЕТ БЫТЬ МЕДЛЕННЕЕ Однопоточной
- (!!!) STL используем чаще и читаем что в ней есть
- (!!!) АЛГОРИТМЫ улучшают решение в 2 и 100 раз порой

### RACE CONDITION / RACE_CONDITION / RACECONDITION:
	deq.pop_front() // while deq is empty
Это поток пришёл делать pop_front из пустого дека - ПРИЧИНА - Так сложилось планирование потока

### DATA RACE / DATARACE / DATA_RACE
	shared_counter++;
ОДНА ячейка памяти и мы с ней работаем без объектов синхронизации из разных потоков, и в некоторых потоках мы пишем
...
## Проблемы многопоточности Multithreading problems

- **Оговорка 1** - Lock-free даёт выигрыш при параллельном программировании с сильной гранулярностью задач, когда расходы на традиционную синхронизацию велики.
- **Оговорка 2** - Одновременный доступ к атомарному значению существенно снижает производительность. Может так оказаться, что однопоточная реализация алгоритма окажется быстрее многопоточного из-за отсутствия накладных расходов. Поэтому таких ситуаций лучше избегать совсем.

1) **Deadlock (Взаимная блокировка) dead lock**
- Несколько потоков блокируют друг друга. Программа именно "зависла", как правило именно встала и стоит, т.е. никто ничего не делает
- Пример: Два потока входят одновременно в 2 функции (каждый в свою) и в ф-циях лочатся 2 мьютекса не в одинаковом порядке. Один поток залочит 1, второй поток залочил 2, 1 хочет залочить 2 но он уже залочен, и 2 хочет залочить 1, но он уже залочен, оба ждут разлока до бесконечности.
- РЕШЕНИЯ Двух типов:
1. Что-то делать с блокировками, *РЕШ_1.1* сверять порядок, и другие Р_1.2, Р_1.3, Р_1.4, см. ниже.
2. ОТКАЗАТЬСЯ от блокировок РЕШ_2, см. ниже (к примеру в пользу Неблокирующей очереди сообщений | Non blocking Message queue - Пример 1: Qt-шный цикл обработки сообщений event loop; Пример 2: WINDOWS Style Gui event dispatching and handling через Event Message Queue)

- *РЕШЕНИЕ 1.1*: Использовать одинаковый порядок lock/unlock Блокировки и Разброкировки во всех методах и операциях с мьютексами. Использовать сложные мьютексы которые можно лочить более одного раза. Использовать Спин_локи
- РЕШЕНИЕ 1.2: УМЕНЬШЕНИЕ числа вложенных блокировок впринципе
- РЕШЕНИЕ 1.3: Разблокировка через **ТАЙМАУТ для РАЗблокировки** - следим (нпример третьим потоком или ещё как-то) за временем сколько блокировка уже захвачена и находится в состоянии Блокировки и разлочиваем, если слишком долго.
...
- Оба потока делают одно и тоже, проверяют одну и ту же переменную, которая никак не может поменяться, ибо пока мы не выйдем из одного из них она не поменяется, но каждый раз когда выходит первый поток, второй поток успевает удержать её в исходном состоянии и так делает каждый из потоков и они формально выполняют свою работу, но условие выхода для них никогда не настаёт. 2 Человека пытаются пройти через проход, но каждый одновременно делает шаг и в итоге ни кто из них не может сделать первй шаг.
- РЕШЕНИЕ: Переписывать логику процесса 
Источник: https://ios-interview.ru/multithreading-problems/
3) **Starvation (Голодание потоков)**
- Поток(и) "перетягивают на себя одеяло", т.е. не все, а один или несколько потоков мешают одному или нескольким другим потокам выполнять их работу так, как это задумывалось разработчиком (то есть наиболее эффективно).
...
4) **Race condition (Состояние гонки) racecondition**
- 2 потока получают доступ к общим данным и они пытаются эти данные одновременно изменить. Итоговое состояние данных зависит от того "Кто быстрее того и тапки", в каком порядке потоки получили доступ к данным. Когда время или порядок событий влияют на корректность результата. Непредсказуемость конечного состояния данных.
- РЕШЕНИЕ 1: **АТОМАРНЫЕ операции** - Если операция НЕ Атомарна — то она состоит из нескольких шагов. Во время АТОМАРНОЙ операции НИКТО не может "втиснуться" / "вклиниться всередину" перед ней или выполниться одновременно с нею.
- РЕШЕНИЕ 2: Блокировки потоков, которые обгоняют другие потоки, в не нужный для нас момент, построение очереди из потокв для верной последовательности операция
5) **Data Race (Гонка за данными) datarace**
- Один поток ПИШЕТ, второй поток одновременно ЧИТАЕТ. Один поток обращается к изменяемому объекту, в то время как другой поток записывает в него.
- РЕШЕНИЕ 1: **АТОМАРНЫЕ операции** - Если операция НЕ Атомарна — то она состоит из нескольких шагов. Во время АТОМАРНОЙ операции НИКТО не может "втиснуться" / "вклиниться всередину" перед ней или выполниться одновременно с нею.
- РЕШЕНИЕ 2: Блокировки через Мьютексы=std::mutex=NSLock(lightweight-lock with .lock()/.unlock())/Крит.секции/Локи/Спин-локи/Кондвары/Semaphore (smfr; lock(){smfr.wait()}; unlock(){smfr.signal()}/Serial queue и т.д. потоков, которые обгоняют другие потоки, в не нужный для нас момент, построение очереди из потокв для верной последовательности операция
6) **Priority inversion (Инверсия приоритетов)**
...
### Дедлоки (avoiding circular wait)
- решается мьютексами
...
ЗАЧЕМ:
- ТИПИЧНЫЙ и почти КАНОНИЧНЫЙ ПРИМЕР - пишем свой ВЕКТОР, и ХОТИМ, чтобы все выделенные объекты лежали последовательно друг за другом, для этого placement-new-каем их последовательно каждый друг за другом, чтобы вектор представлял из себя один смежный участок памяти
...
### [Q] Как запретить копирование объекта класса / Как защитить объект от копирования?
...
int r1 = reinterpret_cast<B *>(&c)->b; // 1
  ...
  // РАБОТАЕТ В ОБЕ СТОРОНЫ преобразуя и вниз и вверх, НО
...
int r2 = static_cast<B *>(&c)->b; // 2
  ...
  // проверки на этапе выполнения
  // РАБОТАЕТ В ОБЕ СТОРОНЫ преобразуя и вниз и вверх, НО
  // его наличие зачастую говорит про плохой дизайн приложения
  // бросает throw std::bad_cast, при преобразовании к ссылке
  // dynamic_cast is useful when you don't know what 
  //the dynamic type of the object is. 
  // It returns a null pointer if the object referred 
  //to doesn't contain the type casted to as a base class
  // You CAN NOT use dynamic_cast for downcast (casting to a derived class)
  // if the argument type is not polymorphic.
...
- Windows SystemAdministartor angefangen zu arbeiten. Meine Arbeit bestand darin dass ich Oper.Systeme auf die PCs installiert habe, Drucker und andere Geräte angegschlossen, Einrichtung lokaler Netze, etwas ingenior Arbeit. Ich habe mich hier mit BorlandC++ Builder begegnet. Es war Programieren, aber auf einem niedriegem Level und aber die erste Begegnung mit c++ Sprache und mit IDE und GUI Konstruktor.
...
Man branchte mir bei wie man auf reinem C code einige Teile des Analyse-system man schreiben kann. Wie man UnitTests für das Analzse System schreiben kann. Man zeigte uns Suchalgorythmen, die wir analisieren mussten und sehr viel debugging von code gemacht um Patterns zu entdecken. 

Ich konnte nich angestellt werden, da nur Senior Positionen waren in der Firma offen, und ich hatte noch nicht genug ERFAHRUNG.
SW Enwicklung hat mir noch ab der Uni gefallen. Dort war beides interssant – sowohl das obeflächlicke auf der Client seite alsauch die Backend seite der programmierung. Der fakt das man mit code etwas erschaffen kann und das es funkzioniert und alles was man dafür braucht ist – nur code zu schreiben. Obwohl wir in dem 1-sten StudienJar nur rheines C, Pascal und Visual Basic hatten, trotzdem, als man mir gezeigt hat wie man diese Basis Kentnisse in der Realität nutzt und das man damit große software produkte ertellen kann, ich wollte nur mehr und tiefer mich damit beschäftigen und weiter als SWE zu arbeiten.

- Als SW Enwitckler habe in der Firma Symbol Designe (Task in Task out, Kanban) 3 С++ 1 Database 1 Tester Analytiker - Tanklager
...
- Implementierung der Multithreaded-Thick-Client-Anwendung, die das OPC-Protokoll verwendete, um den Tanken Prozess an der Tankstelle zu kontrollieren und Windows-Sockets, um Statistiken an einen externen Microservice zu senden.
- Entwicklung von zusätzlichen Tools auf der Client-Seite, um mit Einstellungen in XML-, JSON-, INI-Formaten und der MSSQL Server Datenbank zu arbeiten.
