...
### Что будет, если std::thread не выполнить join()
- Программа упадёт, ДОПИСАТЬ
...
- имя объекта
- Класс (ID) защиты объекта
- счетчик количества пользователей объекта
...
- данные, потребляющие ресурсы ОС 3, то есть данные проецируемые на виртуальное адресное пространство
- СТЕК программы
- состояния СЧЁТЧИКОВ и связанный набор РЕГИСТРОВ (Пример: счётчик команд, регистр с адресами команд(ы), которые стоят следующие(ая) на исполнение) 
...
- Имеет набор регистров процессора
- Имеет 2 стека: 1-ый Юзер-модный 2-ой Кернел-модный. Для работы в 1. юзер-мод режиме и для работы в 2. кернел-мод режиме
- Стек потока так же содержит все функции и локальные переменные для отладки кода 
- И так же содержит в себе TLS ThreadLocalStorage - локальная память потока
