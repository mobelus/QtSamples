...
- РЕШЕНИЯ Двух типов:
1. Что-то делать с блокировками, сверять порядок
2. ОТКАЗАТЬСЯ от блокировок (к примеру в пользу Неблокирующей очереди сообщений | Non blocking Message queue - Пример 1: Qt-шный цикл обработки сообщений event loop; Пример 2: WINDOWS Style Gui event dispatching and handling через Event Message Queue)
...
- РЕШЕНИЕ 2.1: **LOCK-FREE Алгоритмы lock free** -
0. Неблокирующая очередь сообщений | Non blocking Message queue
1. Операции атомарного изменения (RMW, read-modify-write)
2. Циклы Compare-And-Swap (CAS)
3. Последовательная согласованность (sequential consistency)
4. Упорядочение памяти (memory reordering)
