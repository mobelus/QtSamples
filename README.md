...
- ОБЩЕЕ для умных указателей - они RAII обёртка для сырыхз указателей (деструктор вызывается, объект удаляется)
...
- А ЕСЛИ мы создали make_shared наш объект, то даже если счётчик Cильных ссылок стал ноль, а weak_pointer-счетчик ссылок !=0 (weak-ссылки ещё остались), **ТО ОБЪЕКТ НЕ УДАЛИТСЯ**, ИБО у нас была одна общая аллокация. Другими словами  make_shared() удалит объект только тогда, когда ОБА счётчика (Счётчик сильных и Счётчик слабых ссылок) равны нулю.
...
- declvalue используется внутри decltype, т.е. в невычисляемом контексте. Никакого фактического "вызова метода" там не будет. declvalue позволяет лишь сформировать корректное выражение, содержащее вызов метода, но оно не позволяет фактически вызвать этот метод.
...
// Если бы volatile отсутствовало, то 
// компилятор мог бы переделать код на что-то аля:
if(i == 1) // Нет необходимости проверять 
// i все время, если и так известно, что оно не изменяется
```
===================================================================
...
- При любой операции вставки (в любое место) push_back / insert ЕСЛИ size стал > capacity, то случится РЕАЛЛОКАЦИЯ с расширением capacity вектора в 2 раза (зависит от версии компилятора, но коэффициент расширения примерно равен двум) и с полным перекопированием всего вектора, в новую реаллоцированную память и с очисткой старой памяти. Только (разростание вектора) выход за capacity вызывает РЕАЛЛОКАЦИЮ и перекопирование всего вектора, остальные случаи/операции не вынуждают нас копировать весь вектор.
- Вставка в начало O(n), если мы не вышли за capacity, вызовет "сдвиг вправо на 1 эелемент всего вектора в рамках capacity", то есть копирование всех элементов вектора на одну позицию вправо, чтобы потом затереть старый первый элемент и поместить на его место новый первый, что мы хотели вставить. insert(v=9,pos=0) 1. v={2,3,4} 2. v{2,2,3,4} 3. v={9,2,3,4}
- Вставка в середину O(n), если мы не вышли за capacity, вызовет сдвиг вправо относительно места вставки всех элементов, и размещение нового элемента на месте строго. insert(v=9,pos=1) 1. v={2,3,4} 2. v{2,3,3,4} 3. v={2,9,3,4}
- Удаление из начала вызовет сдвиг и копирование всех элементов за О(n), которые были справа, на одну позицию влево.
- Удаление из середины вызовет частичный сдвиг и копирование всех элементов за О(n), которые были справа от удаляемого, на одну позицию влево.
- Вставка в КОНЕЦ вектора занимет - **АМОРТИЗИРОВАННОЕ** O(1), ибо пока вектор не нужно расширять (size <= capacity), то новый элемент за О(1) помещается в конец вектора, как только надо вставить элемент в конец, когда capacity уже не хватает, вектор РЕАЛЛОЦИРУЕТСЯ (выделяет себе памяти в ДВА раза больше старого capacity, и копирует все значения из старого места (после копирования удаляет старую память) в новое реаллоцированное (вызываются конструкторы копии у элементов)), что может привести к оценке скорости в О(n), поэтому вектору стоит делать **RESERVE()** если есть понимание на сколько большой он нужен заранее.
...
- ИБО vector реализует СТРОГУЮ Гарантию Безопасности исключений, поэтому нельзя просто так move-ать его элементы
...
- Классы-Обёртки-Для-Лок-Механизмов lock_guard, unique_lock, scoped_lock
...
- **Помним 4-тое** - **NEW и исключение в конструкторе** new реализует СТРОГУЮ ГАРАНТИЮ ИСКЛЮЧЕНИЙ, см. ниже
...
- ЕСЛИ мы для NEW 1. выделили память malloc(sizeof(MyType)) (if ERROR => bad_alloc exception) и 2. ctor() выбросил исключение, ТО new гарантирует нам, освобождение этой памяти

```
class A { A() { throw 1; } };
void main(){
  try {
    // new: 1. allocation of memory for A with malloc(sizeof(A))
...
std::uncaught_exception() - Позволяет проверить летит ли прямо сейчас эксепшн или нет
	
### Как получить исключение / поймать исключение, вылетевшее в другом потоке ?
- 10 потоков воркеров, в одном вылетело исключение в основном потоке я хочу проверить вылетело ли где-то исключение или нет.
- Сохранить вылетевшие исключение мы сможем через **STD::EXCEPTION_PTR**
- В std::promise мы можем сделать set_exception
- И когда мы вызовем std::future().GET(), исключение из future->promise->get() выброситься и мы сможем его перехватить и обработать в try-catch-блоке
...
### [Q] 47. Есть конструктор с один параметром и тип этого параметра не совпадает с типом класса ? Какая есть особенность у ТАКОГО конструтора с один параметром ?
- Он может неявно вызываться, при инициализации
Ответ: При работе с таким классмо как с параметром функции void fun(B b), Будут при копировании B obj2 = 10; и fun(30); присходить IMPLICIT CALL-ы, т.е. НЕЯВНОЕ ПРЕОБРАЗОВАНИЕ и вызов Конструктора с параметром (ПРИМЕР см. 3 вопросами ниже)
Нюанс: Запретить такое поведение можно, используя ключевое слово EXPLICIT (ПРИМЕР см. 2 вопросами ниже)
...
- В С нет исключений. НО есть setjump и longjump - и можно при помощи этих функций их реализовать при целой куче оговорок, но МОЖНО

В каком случае возможна утечка памяти для умного указателя.
ОТВЕТ: Случай когда мы создаваемый умный указатель передаём как параметр аргумента функции и есть ещё один параметр функции, котрый является вызовом функции func( shared_ptr<A> (new A) , call() ); call если бросит исключение, то память УТЕЧЁТ !

- В Умных указателях есть DELETER-ы
- Т.к. умные указатели реализуют RAII, то мы можем не просто работу с памятью арганизовывать, а так же например
Работу с файлом. Описать процесс открытия ФАЙЛА в умном указателе, в ДЕЛИТЕРЕ описать процесс закрытия файла
	
Хотим написать Умный указатель и хотим чтобы он корректно работал как для одного объекта, так и для массива объектов, но не хотим для этого использовать vector, как мы для этого можем использовать DELETER
- new int(1)
- new int[10]
