...
```
Семафор: - примитив синхронизации, разрешающий конкретному числу потоков одновременную работу c неким общим ресурсом. Для него задаётся 2 параметра, сколько сколько потоков могут получить доступ к ресурсу одновременно, и каково общее число потоков, которое начнёт параллельную работу.
std::counting_semaphore, std::binary_semaphore

### spinlock spin_lock поменьше
...
- Критическая секция не вызывает переключения контекста из Пользовательского режима в Режим Ядра.
- Критическая секция - Крит секция это объект ПОЛЬЗОВАТЕЛЬСКОГО режима. А Мьютекс это объект ЯДРА.
...
- ОГОВОРКА: ЕСЛИ есть Критическая Секция - множество потоков пытаются в неё войти, а кто-то один её залочил и потоки ждут ДОЛГО. Так как "ПОД КАПОТОМ" этой Крит.Секции спрятан Мьютекс - Который ЗАХВАТЫВАЕТСЯ тогда, когда уровень критического ожидания входа в Крит.Секцию является Достаточно большим.
...
### Разница между uniqie_ptr и shared_ptr
...
- uniqie_ptr - умный указатель БЕЗ счёт он сам уникально владеет объектом (когда наоборот хотим не шарить объект а гарантировать монопольное использование)
