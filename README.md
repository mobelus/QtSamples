...
```	
0) он передаст параметры в конструктор своего параметра шаблонного some_struct и там же внутри вызовет placement_new, чтобы выделить память под объект на месте по известному адресу памяти.
1) + У нас пропадает вызов **new**, который имеет место быть, когда мы используем обычный  std::shared_ptr<int> x(new int);
2) + Нет проблемы с брошенным исключением при вычислении аргументов функции - foo(shared_ptr<A>(new A), bar()). Порядок вызова аргументов и последовательность вызова bar() / new A / shared_ptr не определена строго стандартом, так что Если вызвалось 1. new A 2. вызвалось bar() и bar() вдруг бросит исключение и т.к. конструктор shared_ptr ещё не вызывался, вызвался конструктор только самого объекта А, и память выделенная под А утечёт
3) + Выделяет память на счётчик ссылок и на объект одним блоком. Меньше проблем с фрагментацией памяти, всё расположено близко, а значит всё более дефрагментированно хранится
...
# Vector
- В основе - динамический массив, занимает смежный или непрерывный участок в памяти, с О(1) доступ к элементам по индексу 
- Вставка/Удаление в середину/начало вызовет перемещение и копирование всех элементов, которые были справа, если вылезем за capacity, это вызовет реаллокация всего вектора целиком.
- Вставка в КОНЕЦ вектора занимет - **АМОРТИЗИРОВАННОЕ** O(1), ибо пока вектор не нужно расширять (size <= capacity), то новый элемент за О(1) помещается в конец вектора, как только надо вставить элемент в конец, когда capacity уже не хватает, вектор РЕАЛЛОЦИРУЕТСЯ (выделяет себе памяти в два раза больше старого capacity, и копирует все значения из старого места в новое реаллоцированное), что может привести к оценке скорости в О(n), поэтому вектору стоит делать **RESERVE()** если есть понимание на сколько большой он нужен заранее.
...
- В основе - "список фиксированных векторов" - динамический массив, лежит кусками по несколько элементов, каждый кусок это условный "вектор" с начальным элементом и с конечным элементом, в которых находятся указатели на начало и конец другого куска. Быстрая за О(1) вставка в начало и в конец.
...
# List
- В основе - динамический массив, но каждый элемент в произвольных местах в памяти, с двумя (list двусвязный) или одним (forward_list односвязный) указателями на предыдущий+следующий элементы или только на следующий элемент. (ПЛЮС forward_list, занимает меньше места в памяти)
...
- 1) ЕСЛИ потребуется перестройка хеш-таблицы время вставки будет линейным O(n)
- 2) ЕСЛИ будут коллизии (разные входные данные, для которых хэш-функция выдала одинаковый хеш), и таких случаев много, то на месте коллизий начинают формироваться 'БАКЕТЫ', они как 
